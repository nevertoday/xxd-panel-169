...
# XXD Panel 169｜따뜻한 황토색 작은 잉크 삽화
...
서로 다른 원본에서 각각 생성한 Panel 169의 첫 시도 샘플 8장입니다. 짧은 영어 문구를 사용했습니다. 가로는 1536×864 좌우 균등 분할, 세로는 1152×1536 상하 균등 분할입니다.

따뜻한 흰 종이, 독립된 잉크 삽화와 원본 대응을 모두 확인했습니다. 06·09에는 파란색이 섞였고 07·12는 풍경을 많이 유지하며 08은 선이 굵고 만화적입니다. 일부 삽화나 글씨가 커서 제한된 색상, 가는 먹선, 75–85% 여백 조건을 일관되게 충족하지 못합니다. 첫 시도의 실제 결과이며 모두 합격한 스타일 예시는 아닙니다.
...
75–85%는 그리지 않은 따뜻한 흰 종이, 15–25%는 작은 독립 삽화입니다. 가는 먹선·황토·카키·소량의 구운 주황만 씁니다.
...
- `design-only`: 전체 캔버스에 Panel 169의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-169`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-169 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-169-prompt.en.md)와 [중국어](references/xxd-panel-169-prompt.zh-CN.md)를 확인하세요.
